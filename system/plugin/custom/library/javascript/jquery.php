 ...
 * Jquery Class
 */
class AS_Jquery extends AS_Javascript {
	...
	protected function _updater($container = 'this', $controller = '', $options = '')
	{
		...
		$controller = (strpos('://', $controller) === FALSE) ? $controller : $this->AS->config->site_url($controller);

		// ajaxStart and ajaxStop are better choices here... but this is a stop gap
		if ($this->AS->config->item('javascript_ajax_img') === '')
		{
			$loading_notifier = 'Loading...';
		}
		else
		{
			$loading_notifier = '<img src="'.$this->AS->config->slash_item('base_url').$this->AS->config->item('javascript_ajax_img').'" alt="Loading" />';
		}
	...
	protected function _compile($view_var = 'script_foot', $script_tags = TRUE)
	{
		...
		$this->AS->load->vars(array('library_src' => $external_scripts));
		...
		$this->AS->load->vars(array($view_var => $output));
	}
