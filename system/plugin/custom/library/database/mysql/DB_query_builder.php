...
abstract class AS_DB_query_builder extends AS_DB_driver {
	...
	protected function _compile_wh($qb_key)
	{
		if (count($this->$qb_key) > 0)
		{
			for ($i = 0, $c = count($this->$qb_key); $i < $c; $i++)
			{
				...
				for ($as = 0, $cc = count($conditions); $as < $cc; $as++)
				{
					if (($op = $this->_get_operator($conditions[$as])) === FALSE
						OR ! preg_match('/^(\(?)(.*)('.preg_quote($op, '/').')\s*(.*(?<!\)))?(\)?)$/i', $conditions[$as], $matches))
					{
						continue;
					}
					...
					$conditions[$as] = $matches[1].$this->protect_identifiers(trim($matches[2]))
