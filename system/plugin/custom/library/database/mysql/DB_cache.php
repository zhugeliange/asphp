...
class AS_DB_Cache {
	...
	{
		// Assign the main AS object to $this->AS and load the file helper since we use it a lot
		$this->AS =& get_instance();
		$this->db =& $db;
		$this->AS->load->helper('file');
	...
	{
		$segment_one = ($this->AS->uri->segment(1) == FALSE) ? 'default' : $this->AS->uri->segment(1);
		$segment_two = ($this->AS->uri->segment(2) == FALSE) ? 'index' : $this->AS->uri->segment(2);
