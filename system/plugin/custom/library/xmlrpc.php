...
class AS_Xmlrpc {
	 ...
	 * @var	string
	 */
	public $xmlrpcName		= 'XML-RPC for AS';
 ...
 */
class XML_RPC_Client extends AS_Xmlrpc
{
	/**
	 * Path
	 *
	...
	public function decode($array = NULL)
	{
		...
		if (is_array($array))
		{
			foreach ($array as $key => &$value)
			{
				...
				{
					$array[$key] = $AS->security->xss_clean($value);
				}
			}
		...
		elseif ($this->xss_clean)
		{
			$result = $AS->security->xss_clean($result);
		}
 ...
 */
class XML_RPC_Message extends AS_Xmlrpc
{
	...
	public function output_parameters(array $array = array())
	{
		...
		for ($i = 0, $c = count($this->params); $i < $c; $i++)
		{
			...
			{
				$parameters[] = ($this->xss_clean) ? $AS->security->xss_clean($a_param) : $a_param;
			}
		}
 ...
 */
class XML_RPC_Values extends AS_Xmlrpc
{
	/**
	 * Value data
	 *
