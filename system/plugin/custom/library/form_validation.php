 ...
 */
class AS_Form_validation {

	/**
	 * Reference to the AS instance
	 *
	 * @var object
	 */
	protected $AS;
	 ...
	 * @param	mixed	$rules
	 * @param	array	$errors
	 * @return	AS_Form_validation
	 */
	public function set_rules($field, $label = '', $rules = array(), $errors = array())
	{
		// No reason to set rules if we have no POST data
		// or a validation array has not been specified
		if ($this->AS->input->method() !== 'post' && empty($this->validation_data))
		{
			return $this;
		}
	 ...
	 * each array due to the limitations of AS's singleton
	 *
	 * @param	array	$data
	 * @return	AS_Form_validation
	 */
	...
	public function run($group = '')
	{
		...
		if (count($this->_field_data) === 0)
		{
			...
			if (empty($group))
			{
				...
				$group = trim($this->AS->uri->ruri_string(), '/');
				isset($this->_config_rules[$group]) OR $group = $this->AS->router->class.'/'.$this->AS->router->method;
			}
		...
		$this->AS->lang->load('form_validation');
	...
	protected function _execute($row, $rules, $postdata = NULL, $cycles = 0)
	{
		...
		foreach ($rules as $rule)
		{
			...
			if ($callback OR $callable !== FALSE)
			{
				if ($callback)
				{
					...
					else
					{
						...
						$result = $this->AS->$rule($postdata, $param);
					}
				}
				else
				{
			...
			if ($result === FALSE)
			{
				...
				if ( ! is_string($rule))
				{
					$line = $this->AS->lang->line('form_validation_error_message_not_set').'(Anonymous function)';
				}
				else
				{
	...
	protected function _get_error_message($rule, $field)
	{
		...
		elseif (FALSE !== ($line = $this->AS->lang->line('form_validation_'.$rule)))
		{
			return $line;
		}
		// DEPRECATED support for non-prefixed keys, lang file again
		elseif (FALSE !== ($line = $this->AS->lang->line($rule, FALSE)))
		{
			return $line;
		}

		return $this->AS->lang->line('form_validation_error_message_not_set').'('.$rule.')';
	}
	...
	protected function _translate_fieldname($fieldname)
	{
		...
		if (sscanf($fieldname, 'lang:%s', $line) === 1 && FALSE === ($fieldname = $this->AS->lang->line($line, FALSE)))
		{
			return $line;
		}
	...
	public function is_unique($str, $field)
	{
		...
		return isset($this->AS->db)
			? ($this->AS->db->limit(1)->get_where($table, array($field => $str))->num_rows() === 0)
			: FALSE;
	}
	...
	{
		return $this->AS->input->valid_ip($ip, $which);
	}
	...
	{
		return $this->AS->security->strip_image_tags($str);
	}
	 ...
	 * results of any previous validation routine due to the AS singleton.
	 *
	 * @return	AS_Form_validation
	 */
