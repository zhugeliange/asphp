...
class AS_Unit_test {
	...
	public function report($result = array())
	{
		...
		foreach ($result as $res)
		{
			...
			{
				if ($key === $AS->lang->line('ut_result'))
				{
					if ($val === $AS->lang->line('ut_passed'))
					{
						$val = '<span style="color: #0C0;">'.$val.'</span>';
					}
					elseif ($val === $AS->lang->line('ut_failed'))
					{
	...
	public function result($results = array())
	{
		...
		foreach ($results as $result)
		{
			...
			foreach ($result as $key => $val)
			{
				...
				{
					if (FALSE !== ($line = $AS->lang->line(strtolower('ut_'.$val), FALSE)))
					{
						$val = $line;
					}
				}

				$temp[$AS->lang->line('ut_'.$key, FALSE)] = $val;
			}
