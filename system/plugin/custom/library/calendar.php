...
class AS_Calendar {
	...
	public function initialize($config = array())
	{
		...
		{
			$this->next_prev_url = $this->AS->config->site_url($this->AS->router->class.'/'.$this->AS->router->method);
		}
	...
	public function get_month_name($month)
	{
		...
		return ($this->AS->lang->line($month_names[$month]) === FALSE)
			? ucfirst(substr($month_names[$month], 4))
			: $this->AS->lang->line($month_names[$month]);
	}
	...
	public function get_day_names($day_type = '')
	{
		...
		{
			$days[] = ($this->AS->lang->line('cal_'.$day_names[$i]) === FALSE) ? ucfirst($day_names[$i]) : $this->AS->lang->line('cal_'.$day_names[$i]);
		}
